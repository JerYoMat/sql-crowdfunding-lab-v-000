...
"SELECT projects.title, sum(pledges.amount) from projects join pledges on pledges.project_id = projects.id group by pledges.project_id order by title ASC, order by name ASC;"
